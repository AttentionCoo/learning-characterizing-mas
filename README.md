...
> 文档状态：已于 2026-07-29 按当前代码、Docker 编排和自动化测试核对。系统用于教学辅助，不替代教师指导或临床诊疗意见。
...
| 智能辅导 | 多轮 SSE 问答、历史记录、图片和代码片段上下文 |
...
| 质量控制 | 功能级输入守卫、Hybrid RAG、规则校验、反思修正、辩论仲裁、共享记忆 |
...
| 层级 | 主要技术 |
|:---|:---|
| 前端 | Vue 3.5、Vite 7、Pinia 3、marked 17、DOMPurify、Nginx 1.27 |
| 后端 | Java 21、Spring Boot 3.3.13、Spring MVC/WebFlux、MyBatis-Plus、MySQL 8、Redis 7、阿里云 OSS |
| 模型 | Python 3.11、FastAPI 0.128、LangGraph 1.2、LangChain 1.3、ChromaDB 1.5 |
...
模型服务首次启动需要加载 `model/data/documents/` 中的 PDF，并初始化 BM25 和向量库。查看状态：
...
2. 启动模型：在 `model/` 安装依赖后执行 `python -m app.main`。
...
2026-07-29 本地验证结果：模型层 93 项通过，前端 16 项通过，后端 9 项通过、1 项跳过；合计 118 项通过、1 项跳过。
...
SSE 可能包含 `init`、`node_start`、`node_done`、`token`、`replace`、`done` 和 `error`。`replace` 表示完整报告，应替换此前累计的 `token` 内容。
