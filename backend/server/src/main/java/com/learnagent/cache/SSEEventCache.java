 ...
 * SSE 断线续传事件缓存�?
 *
 * <p>为每�?talkId 维护一�?Reactor replay sink，同时承担两个职责：
 * <ol>
 *   <li>为当次连接的每条事件分配单调递增序列号（seq），注入 SSE id 字段（格�?talkId:seq�?/li>
 *   <li>客户端重连时，通过 seq 过滤出未收到的事件进行回放，
 *       若原始流仍在推送则自动续接直播</li>
 * </ol>
 *
 * <p>缓存生命周期：流结束（done/error）后保留 {@code ai.sse.cache-ttl-minutes} 分钟�?
 * 供客户端在此窗口内重连恢复。窗口过期后�?{@link #cleanExpired()} 定期清理�?
 */
@Slf4j
@Component
public class SSEEventCache {

    /** 携带序列号的事件包装，重连时�?seq 过滤已收到的事件 */
    public record SequencedEvent(long seq, String data) {}

    /** 每个 talkId 的滑动窗口事件数上限，超出后最旧的事件被淘�?*/
    @Value("${ai.sse.ring-buffer-size:200}")
    private int ringBufferSize;

    /** done/error 后缓存保留时长（分钟），给重连留出窗�?*/
    @Value("${ai.sse.cache-ttl-minutes:5}")
    private long cacheTtlMinutes;

    // talkId �?replay sink（流结束后保�?ttl 分钟，供重连回放�?
    ...
    /**
     * 新流开始前调用，创建并注册 replay sink �?seq 计数器�?
     *
     * @param talkId 对话 ID（字符串形式，作为缓�?key�?
     */
    public void registerStream(String talkId) {
        ...
        if (existing != null) {
            ...
            log.debug("替换已有 SSE sink，并设置�?sink 的过期时�? talkId={}, expiresInMin={}", talkId, cacheTtlMinutes);
        }
        ...
        log.debug("已注�?SSE 事件缓存: talkId={}, ringBufferSize={}", talkId, ringBufferSize);
    }
     ...
     *
     * @param talkId 对话 ID
     * @param data   原始 JSON 字符串事�?
     * @return 分配的序列号�?-based）；�?talkId 未注册则返回 -1
     */
    ...
    /**
     * 流结束（正常 done �?error 恢复后）时调用：
     * <ol>
     *   <li>complete sink，通知所有订阅者（含正在等待的重连客户端）流已结束</li>
     *   <li>记录过期时刻，{@link #cleanExpired()} 到期后释放内�?/li>
     * </ol>
     *
     * @param talkId 对话 ID
     */
    public void completeStream(String talkId) {
        ...
        long expiryMs = System.currentTimeMillis() + cacheTtlMinutes * 60_000L;
        expiryTimes.put(talkId, expiryMs);
        log.debug("SSE 流已完成，缓存将�?{} 分钟后过�? talkId={}", cacheTtlMinutes, talkId);
    }
     ...
     * @return {@code null} 表示缓存不存在或已过期，应向客户端返�?E2003�?
    ...
    /**
     * 每分钟扫描一次，清理已超�?ttl �?talkId 缓存，释�?sink 和计数器占用的内存�?
     */
    ...
    public void cleanExpired() {
        ...
        expiryTimes.entrySet().removeIf(entry -> {
            if (entry.getValue() <= now) {
                ...
                log.debug("已清理过�?SSE 事件缓存: talkId={}", tid);
                return true;
            }
            return false;
        });
