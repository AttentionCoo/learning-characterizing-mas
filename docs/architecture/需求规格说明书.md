...
2. 大模型服务依赖讯飞星火 API（XF-Xinghuo-Max/Plus/Turbo）
3. 多模态能力依赖 xf-xinghuo-vl-max 视觉大模型
5. 文件存储依赖阿里云 OSS
...
|:---|:---|
| **优先级** | 高 |
| **描述** | 基于 xf-xinghuo-vl-max 多模态视觉大模型，流式输出分析结果，无缝融入主对话流式管道。 |
...
| 接口 | 提供方 | 协议 | 用途 |
|:---|:---|:---|:---|
| DashScope API | 阿里云 | HTTPS | XF-Xinghuo-Max/Plus/Turbo LLM 调用 |
| xf-xinghuo-vl-max API | 阿里云 | HTTPS | 多模态视觉理解 |
...
flowchart TD
    ...
    RERANK --> M1{模型1<br/>xf-xinghuo-rerank-v1}
    M1 -->|成功| TOPK[取 Top-K 结果]
    M1 -->|失败| M2{模型2<br/>gte-rerank-v2}
    M2 -->|成功| TOPK
    M2 -->|失败| M3{模型3<br/>xf-xinghuo-rerank}
    M3 -->|成功| TOPK
...
graph TB
    ...
    subgraph 外部API["外部 API"]
        DashScope[DashScope API<br/>XF-Xinghuo LLM + Embedding]
...
| 多模态生成 | FR-VISION-01~03 | ✅ xf-xinghuo-vl-max |
