...
> 状态：2026-07-29 按当前代码核对。代码、配置与数据库脚本是实现事实源；需求和竞赛文档用于描述目标与方案，不应覆盖代码事实。
...
- `docs/architecture/` 下的同名文件只保留导航说明，不复制正文。
- `.docx` 和 `.pptx` 是提交/演示快照，不会随代码自动同步；使用前应根据本页权威 Markdown 复核。
